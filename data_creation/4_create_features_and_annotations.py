...
raw_inertial_folder = './data/wear/raw/inertial/{}hz'.format(sampling_rate)
inertial_folder = './data/wear/processed/inertial_features/{}hz/{}_frames_{}_stride'.format(sampling_rate, frames, stride)
i3d_folder = './data/wear/processed/i3d_features/{}fps/{}_frames_{}_stride'.format(fps, frames, stride)
combined_folder = './data/wear/processed/combined_features/{}fps/{}_frames_{}_stride'.format(fps, frames, stride)
anno_folder = './data/wear/annotations/{}fps'.format(fps)

for i, split_sbjs in enumerate(sbjs):
    wear_annotations = {'version': 'Wear', 'database': {}, 'label_dict': label_dict}
    for sbj in split_sbjs:
        raw_inertial_sbj = pd.read_csv(os.path.join(raw_inertial_folder, sbj + '.csv'), index_col=None, low_memory=False)        
        ...
        output_combined = np.concatenate((output_inertial[:len(output_i3d)], output_i3d), axis=1)
        if not os.path.exists(inertial_folder):
            os.makedirs(inertial_folder)
        np.save(os.path.join(inertial_folder, sbj + '.npy'), output_inertial)
        if not os.path.exists(i3d_folder):
            os.makedirs(i3d_folder)
        np.save(os.path.join(combined_folder, sbj + '.npy'), output_combined)

    # create video annotations
    for j in range(nb_sbjs):
        curr_sbj = "sbj_" + str(j)
        raw_inertial_sbj_t = pd.read_csv(os.path.join(raw_inertial_folder, curr_sbj + '.csv'), index_col=None, low_memory=False)
