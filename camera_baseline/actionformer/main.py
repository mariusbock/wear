...
from camera_baseline.actionformer.libs.datasets.datasets import make_data_loader, make_dataset
from camera_baseline.actionformer.libs.utils.train_utils import valid_one_epoch
from camera_baseline.actionformer.libs.core.config import _update_config


def run_actionformer(cfg, ckpt_folder, ckpt_freq, resume, rng_generator, run):
    ...
    val_sens_data = None    
    val_sbjs = val_dataset.get_subjects()
    for sbj in val_sbjs:
        if cfg['dataset']['include_null']:
            sbj_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], sbj + '.csv'), index_col=False).replace({"label": val_dataset.label_dict}).fillna(0)
        else:
            sbj_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], sbj + '.csv'), index_col=False).replace({"label": val_dataset.label_dict}).fillna(-1)
            sbj_data.iloc[:, -1] += 1 
    ...
    # validation sensor data loading
    train_sens_data = None    
    train_sbjs = train_dataset.get_subjects()
    for sbj in train_sbjs:
        if cfg['dataset']['include_null']:
            sbj_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], sbj + '.csv'), index_col=False).replace({"label": train_dataset.label_dict}).fillna(0)
        else:
            sbj_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], sbj + '.csv'), index_col=False).replace({"label": train_dataset.label_dict}).fillna(-1)
            sbj_data.iloc[:, -1] += 1 
        if train_sens_data is None:
            train_sens_data = sbj_data
        else:
            train_sens_data = pd.concat((train_sens_data, sbj_data))

    train_sens_data = train_sens_data.to_numpy()
    ...
    for epoch in range(start_epoch, start_epoch + max_epochs):
        ...
        det_eval = ANETdetection(val_dataset.json_file, val_dataset.split[0], tiou_thresholds = val_db_vars['tiou_thresholds'])
        
        v_loss, v_segments = valid_one_epoch(val_loader, model)
        v_preds, v_gt, _ = convert_segments_to_samples(v_segments, val_sens_data, cfg['dataset']['sampling_rate'], cfg['dataset']['include_null'])

        if epoch == (start_epoch + max_epochs) - 1:
            ...
            # postprocessing 
            v_preds_post, _, _ = convert_segments_to_samples(v_segments, val_sens_data, cfg['dataset']['sampling_rate'], cfg['dataset']['include_null'], cfg['score_threshold'])
            val_mAP_post, _ = det_eval.evaluate(v_segments, cfg['score_threshold'])
        ...
        block4 = ''
        block4  += '\t\tAvg. mAP {:>4.2f} (%) '.format(np.mean(val_mAP) * 100)
        ...
        block5  += ' F1 {:>4.2f} (%)'.format(np.mean(v_f1) * 100)
        ...
        if run is not None:
            run[split_name].append({"train_loss": t_loss, "val_loss": v_loss, "accuracy": v_acc, "precision": np.mean(v_prec), "recall": np.mean(v_rec), 'f1': np.mean(v_f1), 'mAP': np.mean(val_mAP)}, step=epoch)
            for tiou, tiou_mAP in zip(cfg['dataset']['tiou_thresholds'], val_mAP):
                    run[split_name].append({'mAP@' + str(tiou): tiou_mAP}, step=epoch)

    return t_losses, v_losses, val_mAP, val_mAP_post, v_preds, v_preds_post, v_gt 
